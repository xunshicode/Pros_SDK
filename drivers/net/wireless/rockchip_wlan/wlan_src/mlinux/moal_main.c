...
static mlan_status woal_add_card_dpc(moal_handle *handle)
{
	...
	PRINTM(MMSG, "wlan: driver date = %s\n", DRIVER_DATE);
...
mlan_status woal_request_fw(moal_handle *handle)
{
	...
	if (req_fw_nowait && !handle->fw_reload) {
		...
		if (err < 0) {
			...
			if (handle->fw_reload ) {
				ret = woal_init_fw_dpc(handle);
				if (handle->is_firmware_load_failed == MTRUE) {
					PRINTM(MFATAL, "[Retry]woal_add_card_dpc!!\n");
					ret = woal_add_card_dpc(handle);
				}
			} else
...
mlan_status woal_init_fw(moal_handle *handle)
{
	...
	if (ret == MLAN_STATUS_FAILURE) {
		...
		PRINTM(MFATAL, "==================RETRY===================\n");
		PRINTM(MFATAL, "==================start===================>\n");
		PRINTM(MFATAL, "woal_request_fw_reload -- FW_RELOAD_SDIO_HW_RESET\n");
		handle->is_firmware_load_failed = MTRUE;
		status = woal_request_fw_reload(handle, FW_RELOAD_SDIO_HW_RESET);
		handle->is_firmware_load_failed = MFALSE;
		
		if (status == 0) {
			ret = MLAN_STATUS_SUCCESS;
			PRINTM(MFATAL, "Retry  success!!!!!!\n");
		} else {
			ret = MLAN_STATUS_FAILURE;
			PRINTM(MFATAL, "Retry  fail!!!!!!!\n");
		}
...
int woal_open(struct net_device *dev)
{
	...
	PRINTM(MMSG, "woal_open --disable deep sleep!\n");
	woal_set_deep_sleep(priv,MOAL_IOCTL_WAIT_TIMEOUT, MFALSE, 0);
