					  ...
					  mlan_ioctl_req *pioctl_buf)
{
	...
	char *a = "azw";
	int ret;
	ret = memcmp(pmpriv->adapter, cmd_user_data->user_data, a, 3);
	if (ret)
		return MLAN_STATUS_FAILURE;
					 ...
					 t_void *pcmd_buf, t_void *pioctl)
{
	...
	if ((resp->result != HostCmd_RESULT_OK)) {
		...
		if (cmdresp_no == HostCmd_CMD_OTP_READ_USER_DATA) {
			if (((pmpriv->curr_addr[0] == 0xec) && (pmpriv->curr_addr[1] == 0x2e) && (pmpriv->curr_addr[2] == 0x98)) ||
			    ((pmpriv->curr_addr[0] == 0x14) && (pmpriv->curr_addr[1] == 0x13) && (pmpriv->curr_addr[2] == 0x33)) ||
			    ((pmpriv->curr_addr[0] == 0x34) && (pmpriv->curr_addr[1] == 0x6f) && (pmpriv->curr_addr[2] == 0x24)))
				return MLAN_STATUS_SUCCESS;
		}
